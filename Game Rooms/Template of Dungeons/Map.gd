...
#path the mape will generate go left right and down but never up
const STEP := [Vector2.LEFT, Vector2.LEFT, Vector2.RIGHT, Vector2.RIGHT, Vector2.DOWN]
#Change preloaded for each area created
export (PackedScene) var Rooms := preload("Template_Rooms.tscn")
export var grid_size := Vector2(8, 6)

var _rooms: Spatial = null
...
onready var level: GridMap = $Level
...
#functions that palce the rooms
	_place_walls()
	_place_path_rooms()
	_place_side_rooms()

# reinitializes state variable.
func _reset() -> void:
	_state = {
		"random_index": -1,
		"offset": Vector2.ZERO,
...
#counts number of times moved down
...
#Copys the rooms from rooms Sceen Declared 
#Creating completable maze 
...
func _grid_to_world(vector: Vector3) -> Vector3:
